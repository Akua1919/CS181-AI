...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            trans = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            self.beliefs[position] =  self.beliefs[position] * trans
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for item in newPosDist.items():
                newdistribution[item[0]] += self.beliefs[oldPos] * item[1]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        postuple = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(postuple)
        length = len(postuple)
        for i in range(self.numParticles):
            self.particles.append(postuple[i % length])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                weight *= self.getObservationProb(noisyDistance,  pacmanPosition, particle[i], jailPosition)
            distribution[particle] += weight
