...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        v = self.getQValue(state, action)
        nv = self.computeValueFromQValues(nextState)
        self.values[(state, action)] = (1 - self.alpha) * v + self.alpha * (reward + self.discount * nv)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in featurelist:
            value += featurelist[feature]*self.weights[feature]
        return value
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in featurelist:
            self.weights[feature]  +=  diff  * featurelist[feature]
