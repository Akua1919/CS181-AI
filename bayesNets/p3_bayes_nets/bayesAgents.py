    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if assignment[FOOD_HOUSE_VAR] == getAssignment(obsPos[0] , obsPos[1]):
                ...
                elif assignment[GHOST_HOUSE_VAR] == getAssignment(obsPos[0] , obsPos[1]) :
                    if assignment[obsVar] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    probstand = 0
    mostlikelyhouse = factor.getAllPossibleAssignmentDicts()[0]
    
    for assignment in factor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        if prob > probstand:
            probstand = prob
            mostlikelyhouse = assignment
    return mostlikelyhouse
...
class BayesAgent(game.Agent):
    ...
    def getEvidence(self, gameState):
        evidence = {}
        for ePos, eassignment[obsVar] in gameState.getEvidence().items():
