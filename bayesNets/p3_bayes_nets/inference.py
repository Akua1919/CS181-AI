...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        newFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for eliminatevar in eliminationOrder:
            (newFactors, joinedFactor) = joinFactorsByVariable(newFactors, eliminatevar)

            if len(joinedFactor.unconditionedVariables()) >1.0:
                resultFactor = eliminate(joinedFactor, eliminatevar)
                newFactors.append(resultFactor)

        joinedFactor = joinFactors(newFactors)
        finalFactors = normalize(joinedFactor)
        return finalFactors
